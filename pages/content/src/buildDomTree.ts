...
export const buildDomTree = (
  ...
  function measureTime(fn, metricName) {
    return function (...args) {
      ...
      if (metricName !== 'buildDomTree') {
        // Skip buildDomTree as we measure it separately
  ...
  function buildDomTreeInternal(node, parentIframe = null) {
    ...
    try {
      ...
      measureBuildDomTreePart('attributeProcessing', () => {
        const attributeNames = measureDomOperation(() => node.getAttributeNames?.() || [], 'getAttributeNames');
        for (const name of attributeNames) {
          nodeData.attributes[name] = measureDomOperation(() => node.getAttribute(name), 'getAttribute');
        }
      });
      ...
      measureBuildDomTreePart('childrenProcessing', () => {
        if (node.shadowRoot) {
          ...
          for (const child of node.shadowRoot.childNodes) {
            const domElement = buildDomTreeInternal(child, parentIframe);
            if (domElement) {
            ...
            try {
              ...
              if (iframeDoc) {
                for (const child of iframeDoc.childNodes) {
                  const domElement = buildDomTreeInternal(child, node);
                  if (domElement) {
                    nodeData.children.push(domElement);
                  }
                }
  ...
  const rootId = buildDomTreeInternal(document.body);
